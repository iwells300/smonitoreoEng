...
def recargarDatos():
    ...
    st.session_state['datosWebAtuel'] = archivos


st.sidebar.button("Refresh data", type="primary",on_click=recargarDatos)

###################### Contenexion servidor ###########################
listadoEstaciones=['11415','11428','11435','11403','11453','11448','14404','14417','14411','14416','14410']

with st.spinner('Loading data...'):
   archivos=cargarDatos(listadoEstaciones)
if 'datosWebAtuel' not in st.session_state:        
        st.session_state['datosWebAtuel'] = archivos
...
equ=open('assets/ecuacionesAtuel.json', encoding='utf-8')
...
dataArchivos=[]
for a,nombre in st.session_state.datosWebAtuel:
    #data1= json.load(a)
...
if 'zoomAtuel' not in st.session_state:        
        st.session_state['zoomAtuel'] = 8
if 'latAtuel' not in st.session_state:        
        st.session_state['latAtuel'] = -35.721513
if 'lonAtuel' not in st.session_state:        
        st.session_state['lonAtuel'] = -67.864449

#m=folium.Map(location=[st.session_state.lat,st.session_state.lon],zoom_start=st.session_state.zoom,tiles='https://wms.ign.gob.ar/geoserver/gwc/service/tms/1.0.0/mapabase_topo@EPSG%3A3857@png/{z}/{x}/{-y}.png', attr='Argenmap')
m=folium.Map(location=[st.session_state.latAtuel,st.session_state.lonAtuel],zoom_start=st.session_state.zoomAtuel,tiles=folium.TileLayer('https://wms.ign.gob.ar/geoserver/gwc/service/tms/1.0.0/mapabase_topo@EPSG%3A3857@png/{z}/{x}/{-y}.png', name='Argenmap Topográfico', attr='Argenmap Topográfico'), attr='Argenmap Topográfico')
...
with col1:
    st.markdown("## Atuel River ")       

    options = st.multiselect(
    ' ',
    nombreEstaciones.keys(),
       
    placeholder="Select station", default=list(nombreEstaciones.keys())[3]) 
    if not options:
...
def sliderChange():
    st.session_state['zoomAtuel']=st_data['zoom']    
    st.session_state['latAtuel']=st_data['center']['lat']
    st.session_state['lonAtuel']=st_data['center']['lng']
