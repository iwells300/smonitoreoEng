...
set_bg_hack("image/sali dulce.jpg")


if 'estadoDulce' not in st.session_state:        
        st.session_state['estadoDulce'] = True
...
def cargarDatos(listadoEstaciones):
    ...
    if st.session_state.estadoDulce:
        ...
        st.session_state['estadoDulce'] = False
...
def recargarDatos():
    ...
    st.session_state['datosWebDulce'] = archivos
...
listadoEstaciones=['10474','10473','10472','10471','10470','10475','10802','10824','10852','10850','10823','10821','10817','10820','10825','10206','10409','10410','10411','10481','10421','10419']

with st.spinner('Loading data...'):
   archivos=cargarDatos(listadoEstaciones)
if 'datosWebDulce' not in st.session_state:        
        st.session_state['datosWebDulce'] = archivos
...
equ=open('assets/ecuacionesSaliDulce.json', encoding='utf-8')
...
dataArchivos=[]
for a,nombre in st.session_state.datosWebDulce:
    #data1= json.load(a)
    data1= a
...
if 'zoomDulce' not in st.session_state:        
        st.session_state['zoomDulce'] = 8
if 'latDulce' not in st.session_state:        
        st.session_state['latDulce'] = -27.570721
if 'lonDulce' not in st.session_state:        
        st.session_state['lonDulce'] = -64.888794


m=folium.Map(location=[st.session_state.latDulce,st.session_state.lonDulce],zoom_start=st.session_state.zoomDulce,tiles=folium.TileLayer('https://wms.ign.gob.ar/geoserver/gwc/service/tms/1.0.0/mapabase_topo@EPSG%3A3857@png/{z}/{x}/{-y}.png', name='Argenmap Topográfico', attr='Argenmap Topográfico'), attr='Argenmap Topográfico')
...
with col1:
    st.markdown("## Salí Dulce River ")       

    options = st.multiselect(
    ' ',
    nombreEstaciones.keys(),
       
    placeholder="Seleccionar estacion", default=list(nombreEstaciones.keys())[2]) 
    if not options:
        options=list(list(nombreEstaciones.keys())[3])
...
def sliderChange():
    st.session_state['zoomDulce']=st_data['zoom']    
    st.session_state['latDulce']=st_data['center']['lat']
    st.session_state['lonDulce']=st_data['center']['lng']
...
with col3:
    ...
    for estacion in estacionesDatos:
        ...
        #folium.Circle([-estacion['Latitud'],-estacion['Longitud']], 3000, fill=True,color=color,fill_opacity=1,weight=0).add_child(folium.Popup(pop,max_width=300)).add_to(m)
        #folium.Circle([-estacion['Latitud'],-estacion['Longitud']], 3000, fill=True,color=color,fill_opacity=1,weight=0,popup=folium.Popup(pop,show=True)).add_to(m)
