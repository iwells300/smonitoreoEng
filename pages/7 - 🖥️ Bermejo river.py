...
if 'estadoBermejo' not in st.session_state:        
        st.session_state['estadoBermejo'] = True
...
def cargarDatos(listadoEstaciones):
    ...
    if st.session_state.estadoBermejo:
        ...
        st.session_state['estadoBermejo'] = False
...
def recargarDatos():
    ...
    st.session_state['datosWebBermejo'] = archivos
...
listadoEstaciones=['10607','10605','10604','10693','212410','12602','12610']
with st.spinner('Loading data...'):
   archivos=cargarDatos(listadoEstaciones)
if 'datosWebBermejo' not in st.session_state:        
        st.session_state['datosWebBermejo'] = archivos
...
equ=open('assets/ecuacionesBermejo.json', encoding='utf-8')
...
dataArchivos=[]
for a,nombre in st.session_state.datosWebBermejo:
    #data1= json.load(a)
    data1= a
...
if 'zoomBermejo' not in st.session_state:        
        st.session_state['zoomBermejo'] = 7
if 'latBermejo' not in st.session_state:        
        st.session_state['latBermejo'] = -24.674919208896977 
if 'lonBermejo' not in st.session_state:        
        st.session_state['lonBermejo'] = -61.36804742639314



m=folium.Map(location=[st.session_state.latBermejo,st.session_state.lonBermejo],zoom_start=st.session_state.zoomBermejo,tiles=folium.TileLayer('https://wms.ign.gob.ar/geoserver/gwc/service/tms/1.0.0/mapabase_topo@EPSG%3A3857@png/{z}/{x}/{-y}.png', name='Argenmap Topográfico', attr='Argenmap Topográfico'), attr='Argenmap Topográfico')
...
def sliderChange():
    st.session_state['zoomBermejo']=st_data['zoom']    
    st.session_state['latBermejo']=st_data['center']['lat']
    st.session_state['lonBermejo']=st_data['center']['lng']
